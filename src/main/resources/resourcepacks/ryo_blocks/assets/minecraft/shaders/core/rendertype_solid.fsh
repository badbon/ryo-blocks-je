...
const float KITA_LAVA_OVERLAY_STRENGTH = 0.80;
...
void main() {
    ...
    if (kitaStill || kitaFlow) {
        ...
        float restoredAlpha = vertexColor.a * ColorModulator.a;
        color = vec4(mix(vanilla.rgb, kita.rgb, overlayStrength), restoredAlpha) * lightColor;
    } else {
